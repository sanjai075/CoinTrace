...
import { Store, UserCheck, ShieldAlert, LogOut, ArrowRight } from 'lucide-react';
...
export default async function AcceptInvitePage(props: {
  ...
  // 3. Security: Check if user email matches invitation email
      ...
      <div className="w-full max-w-md p-6 sm:p-8 bg-gray-850 border border-gray-800 rounded-3xl shadow-2xl space-y-6 relative overflow-hidden">
        ...
        <div className="space-y-3">
          ...
          <div className="p-3.5 bg-gray-900/50 rounded-2xl border border-gray-800 text-xs text-gray-400 space-y-1.5">
            ...
            <p><strong>Invited Email:</strong> <span className="text-gray-300 font-semibold">{invitation.email}</span></p>
          </div>
        </div>

        {/* Security / Matching check */}
        {!isEmailMatch ? (
          <div className="p-4 bg-rose-500/10 border border-rose-500/25 rounded-2xl text-xs space-y-3">
            <div className="flex items-start gap-2.5 text-rose-400 font-bold">
              <ShieldAlert className="h-4 w-4 shrink-0 mt-0.5" />
              <span>Email Mismatch</span>
            </div>
            <p className="text-gray-400 leading-relaxed">
              This invitation was sent to <strong className="text-gray-200">{invitation.email}</strong>, but you are currently signed in as <strong className="text-gray-200">{userEmail}</strong>.
            </p>
            <div className="pt-2 border-t border-rose-500/15 flex items-center justify-between">
              ...
              <Link
                href={`/handler/sign-in?next=${encodeURIComponent(`/invite/accept?id=${invitationId}`)}`}
          ...
          <div className="space-y-3 pt-2">
            ...
            <form action={declineStaffInvitation}>
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gray-800 hover:bg-gray-750 text-gray-300 hover:text-white font-semibold rounded-2xl transition-all cursor-pointer text-xs"
              >
                Decline Invitation
              </button>
            </form>
